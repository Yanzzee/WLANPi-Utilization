...
- Retry graph samples are independent one-second ratios retained in the shared
  two-minute history.
- Close a live one-second bucket only after the ordered capture timestamps pass
  its boundary plus the 102.4 ms beacon-delay allowance. A wall-clock UI
  refresh must not finalize a bucket while TShark can still have rows from that
  interval buffered on stdout.
